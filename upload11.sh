...
echo "env['ir.module.module'].search([('state', '=', 'installed'), ('name', 'in', ['add_remove_follower_omax', 'automatic_backup', 'ehcs_signup_captcha'])]).button_immediate_uninstall()" | docker compose run --rm -T odoo11 odoo shell -d ${PGDATABASE}
