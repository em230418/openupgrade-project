...
echo "env['ir.module.module'].search([('name', 'in', ['web_diagram'])]).button_immediate_uninstall()" | docker-compose run --rm odoo14 odoo shell -d korvetlog

# устанавливаем необходимый модуль
# echo "env['ir.module.module'].search([('name', 'in', [])]).button_immediate_install()" | docker-compose run --rm odoo14 odoo shell -d korvetlog
